import { c as createAstro, a as createComponent, f as addAttribute, g as renderScript, b as renderTemplate, m as maybeRenderHead, d as renderComponent, r as renderHead, h as renderSlot } from './astro/server_u1eknVN-.mjs';
import 'kleur/colors';
import 'clsx';
/* empty css                         */
...
const $$MainLayout = createComponent(async ($$result, $$props, $$slots) => {
  ...
  let startColor, endColor, timeInfo, sunPosition, weatherData, pathname;
  try {
    pathname = Astro2.url.pathname;
    const isDev = false;
    console.log("Starting MainLayout initialization");
    try {
      const colors = getSeasonalColors();
      startColor = colors.startColor;
      endColor = colors.endColor;
      console.log("Seasonal colors calculated successfully");
    } catch (error) {
      console.error("Error calculating seasonal colors:", error);
      startColor = "rgb(242, 245, 250)";
      endColor = "rgb(235, 240, 248)";
    }
    try {
      ...
      console.log("Time calculations completed successfully");
    } catch (error) {
      console.error("Error in time calculations:", error);
      throw error;
    }
    try {
      sunPosition = calculateSunPosition(timeInfo.currentHour);
    ...
    try {
      weatherData = await fetchWeatherData("95b20923364d53d85ebe0db1a4d1cc36");
    ...
    console.error("Critical error in MainLayout:", {
